...
public class OneWheelSkateboardController : MonoBehaviour
{
    ...
    // ---------------- NEW: Update-interval ("sensor-to-visual") latency logging ----------------
    [Header("Update-Interval Latency Logging")]
    [Tooltip("If true, logs the time between consecutive Update() calls for latencyMeasurementDuration seconds, starting on Start(), then writes a CSV and stops.")]
    public bool measureUpdateLatency = true;
    [Tooltip("Duration (seconds) to measure for.")]
    public float latencyMeasurementDuration = 10f;
    [Tooltip("Filename prefix. Final name will be <prefix>_<yyyyMMdd_HHmmss>.csv")]
    public string latencyFilePrefix = "SKATEBOARD_SENSOR_LATENCY";

    private bool isMeasuringLatency = false;
    private bool latencyMeasurementSaved = false;
    private float latencyMeasurementStartTime = -1f;
    private float lastUpdateRealtime = -1f;
    private List<float> updateIntervalsMs = new List<float>();
    // ------------------------------------------------------------------------------------------
    ...
    void Start()
    {
        ...
        // ---- Start Update-interval latency measurement ----
        if (measureUpdateLatency)
        {
            updateIntervalsMs.Clear();
            lastUpdateRealtime = Time.realtimeSinceStartup;
            latencyMeasurementStartTime = Time.realtimeSinceStartup;
            isMeasuringLatency = true;
            latencyMeasurementSaved = false;
            Debug.Log($"[LatencyLogger] Started logging Update() intervals for {latencyMeasurementDuration}s.");
        }
    }
    ...
    void Update()
    {
        // ---- Update-interval latency measurement (runs independently of controller logic) ----
        if (isMeasuringLatency)
        {
            float now = Time.realtimeSinceStartup;
            float intervalMs = (now - lastUpdateRealtime) * 1000f;
            updateIntervalsMs.Add(intervalMs);
            lastUpdateRealtime = now;

            if (now - latencyMeasurementStartTime >= latencyMeasurementDuration)
            {
                isMeasuringLatency = false;
                if (!latencyMeasurementSaved)
                {
                    SaveLatencyCsv();
                    latencyMeasurementSaved = true;
                }
            }
        }
        // -----------------------------------------------------------------------------------
    ...
    // ---------------- NEW: save latency CSV ----------------
    private void SaveLatencyCsv()
    {
        try
        {
            string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            string fileName = $"{latencyFilePrefix}_{timestamp}.csv";
            // On Quest this resolves under /sdcard/Android/data/<package>/files/
            string path = Path.Combine(Application.persistentDataPath, fileName);

            using (StreamWriter writer = new StreamWriter(path, false))
            {
                writer.WriteLine("sample_index,update_interval_ms");
                for (int i = 0; i < updateIntervalsMs.Count; i++)
                {
                    writer.WriteLine($"{i},{updateIntervalsMs[i]:F3}");
                }
            }

            Debug.Log($"[LatencyLogger] Saved {updateIntervalsMs.Count} samples to {path}");
        }
        catch (Exception e)
        {
            Debug.LogError($"[LatencyLogger] Failed to save CSV: {e.Message}");
        }
    }
    // ---------------------------------------------------------
